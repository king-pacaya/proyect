document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateVerseLevel = (id, newLevel) => {
        ...
        if (index !== -1) {
            verses[index].lastPracticed = Date.now();
            if (newLevel > 2) {
                verses[index].isDominated = true;
                if (verses[index].currentLevel <= 2) {
                    verses[index].completionCount = (verses[index].completionCount || 0) + 1;
                }
            }
            verses[index].currentLevel = newLevel;
            saveVersesToStorage(verses);
        }
    };
    ...
    const renderSavedVerses = () => {
        ...
        verses.forEach(verse => {
            ...
            const progress = level > 2 ? 100 : (level - 1) * 50;
            const actionButtonHTML = level > 2
            ...
            card.innerHTML = `
                <div class="flex items-start justify-between">
                    <div class="flex items-center gap-3">
                        <span class="iconify text-2xl text-gray-300" data-icon="bi:quote"></span>
                        <h4 class="text-xl font-bold text-gray-800">${verse.reference}</h4>
                    </div>
                    <div class="flex items-center gap-2">
                        ...
                        <div class="relative options-menu">
                            ...
                            <div class="options-menu-dropdown hidden absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-10 border origin-top-right">
                                <a href="#" class="share-verse-btn flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"><span class="iconify" data-icon="material-symbols:share-outline"></span>Compartir</a>
                ...
                <div class="space-y-3">
                    <div class="flex justify-between items-center text-sm font-medium text-gray-500"><span>Progreso de memorización</span><span>${progress}%</span></div>
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex justify-end gap-3 items-center">${actionButtonHTML}</div>
                    </div>
                </div>`;
    ...
    saveModalBtn.addEventListener('click', () => { if (currentFetchedVerse) { const result = saveVerse(currentFetchedVerse); if (result.success) { renderSavedVerses(); closeAddEditModal(); } else { statusMessageEl.textContent = result.message; } } });
    ...
    savedVersesList.addEventListener('click', e => {
        ...
        if (optionsBtn) {
            ...
            document.querySelectorAll('.options-menu-dropdown').forEach(d => d.classList.add('hidden'));
